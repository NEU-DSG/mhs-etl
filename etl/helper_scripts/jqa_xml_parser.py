...
list_person_path = './/ns:list/ns:persRef/[@ref]'
...
def get_peopleList_from_attrValue(ancestor, xpath_as_string, attrib_val_str, namespace):
    ...
    for elem in ancestor.findall('.//ns:item/ns:persRef/[@ref]', namespace):
        person = elem.get(attrib_val_str)
        people_list.append(person)
    for elem in ancestor.findall('.//ns:hi/ns:persRef/[@ref]', namespace):
        person = elem.get(attrib_val_str)
        people_list.append(person)
...
def build_dataframe(xml_dir):
    ...
    #             Call functions.
            ...
            dataframe.append([str(re.sub(r'^[^/]+/', '', file)), entry, date, people, subjects, text])
